...
onMounted(() => {
  ...
  // __flora_editor__ must be set before __flora__ (benchmark waitForReady checks both)
  (window as any).__flora_editor__ = {
    setCamera: (cam: { x: number; y: number; z: number }, _opts?: unknown) => {
      pixiCanvas.value?.setCamera(cam);
    },
  };

  (window as any).__flora__ = {
    getPlantCount: () => docStore.plants.size,
    getShapeCount: () => pixiCanvas.value?.getShapeCount() ?? 0,
    ...
    setRenderMode: (_mode: unknown) => {},  // no-op stub — Pixi has one render mode
    getRenderMode: () => 'pixi',
...
<template>
  <div role="main" style="position: relative; width: 100vw; height: 100vh;">
    <PixiCanvas
      ...
      @ready="handleReady"
    />
    <SelfTestPanel :tti-ms="ttiMs" :get-shape-count="() => pixiCanvas?.getShapeCount() ?? 0" />
    ...
    >
      ...
      <div style="margin-top: 6px; color: #aaa; font-size: 11px;">renderer: Pixi.js v8</div>
    </aside>
  </div>
</template>

...
// Baseline: all plant counts × movement types at zoom 0.08
for (const plantCount of PLANT_COUNTS) {
  for (const movementType of MOVEMENT_TYPES) {
    scenarios.push({ plantCount, zoomLevel: 0.08, movementType });
  }
}
// Zoom levels at 300 plants
for (const zoomLevel of ZOOM_LEVELS) {
  for (const movementType of ['still', 'pan']) {
    scenarios.push({ plantCount: 300, zoomLevel, movementType });
  }
}
...
async function runRep(page, scenario) {
  const { plantCount, zoomLevel, movementType } = scenario;

  // Configure state via window.__flora__
  await page.evaluate(({ plantCount }) => {
    window.__flora__.setPlantCount(plantCount);
  }, { plantCount });

  // Give Vue reactivity + Pixi a frame to settle after plant count change
...
async function main() {
  ...
  // Headed required — Pixi WebGL uses SwiftShader (software) in headless, which tanks FPS.
  // Off-screen: window goes left of display edge; keeps real GPU + rAF timing, no popup.
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      ` [${done}/${scenarios.length}] plants=${scenario.plantCount}` +

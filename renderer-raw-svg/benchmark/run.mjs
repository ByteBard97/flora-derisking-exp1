#!/usr/bin/env node
/**
 * Headless Playwright FPS benchmark for renderer-raw-svg.
 *
 * Usage:
...
const PAN_MODES      = ['container', 'per-shape'];
...
for (const panMode of PAN_MODES) {
  // Baseline matrix: all plant counts, zoom 0.08, all movement types
  for (const plantCount of PLANT_COUNTS) {
    for (const movementType of MOVEMENT_TYPES) {
      scenarios.push({ panMode, plantCount, zoomLevel: 0.08, movementType });
    }
  }

  // Zoom sweep: 300 plants, all zoom levels, still + pan
  for (const zoomLevel of ZOOM_LEVELS) {
    for (const movementType of ['still', 'pan']) {
      scenarios.push({ panMode, plantCount: 300, zoomLevel, movementType });
    }
  }
}
...
async function runRep(page, scenario) {
  const { panMode, plantCount, zoomLevel, movementType } = scenario;

  // Configure state via window.__flora__
  await page.evaluate(({ plantCount, panMode }) => {
    window.__flora__.setPlantCount(plantCount);
    window.__flora__.setRenderMode('svg');   // no-op, keeps parity with exp-2 harness
    window.__flora__.setBackground('none');  // no-op
    window.__flora__.setBgSize(400);         // no-op
    window.__flora__.setPanMode(panMode);
  }, { plantCount, panMode });

  // Give Vue reactivity a frame to settle after plant count change
...
async function main() {
  console.log(`\nrenderer-raw-svg benchmark`);
  console.log(`URL:       ${BASE_URL}`);
  console.log(`Reps:      ${REPS}`);
  console.log(`Scenarios: ${scenarios.length}`);
  console.log(`Est. time: ~${Math.ceil(scenarios.length * REPS * (DURATION_MS + 1000) / 60000)} min\n`);

  // Must run headed — rAF rate is throttled in headless Chromium background tabs.
  const browser = await chromium.launch({
    headless: false,
    // Off-screen: keeps real GPU + rAF timing but window doesn't pop up on your display.
    // Pure Vue/DOM renderer — no veaury requirement for headed mode.
    args: ['--no-sandbox', '--window-position=-2000,0', '--window-size=1440,900'],
  });
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      ` [${done}/${scenarios.length}] panMode=${scenario.panMode} plants=${scenario.plantCount}` +
      ` zoom=${scenario.zoomLevel} move=${scenario.movementType}` +
  ...
  writeFileSync(OUT_PATH, JSON.stringify({
    meta: { url: BASE_URL, reps: REPS, durationMs: DURATION_MS, ts: new Date().toISOString() },
    results,
  }, null, 2));

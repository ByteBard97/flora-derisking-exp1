...
async function runAll() {
  ...
  // M7 — consistency: after 5 drag+undo cycles, plant count === SVG shape count
  ...
  if (plantCount !== shapeCountVal) {
    failures.push(`plant count ${plantCount} ≠ SVG shape count ${shapeCountVal}`);
  }
...
function saveToLocalStorage() {
  ...
  const existing: StoredRun[] = JSON.parse(localStorage.getItem('renderer-raw-svg-results') ?? '[]');
  existing.unshift(run);
  localStorage.setItem('renderer-raw-svg-results', JSON.stringify(existing.slice(0, 5)));
}

function exportJSON() {
  const data = {
    experiment: 'renderer-raw-svg',
    schemaVersion: '1.0',
    date: new Date().toISOString(),
    machine,
  ...
  a.href = url;
  a.download = `renderer-raw-svg-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
  ...
  <div style="position: absolute; top: 12px; right: 12px; z-index: 100; font-family: monospace; font-size: 12px;">
    ...
    <div v-if="open" data-testid="self-test-panel" style="margin-top: 8px; background: #0d150d; color: #ccc; padding: 16px; border-radius: 6px; width: 420px; border: 1px solid #2a4a2a; box-shadow: 0 4px 20px rgba(0,0,0,0.6);">
      ...
      <div style="font-size: 11px; color: #5a8a5a; margin-bottom: 12px; padding-bottom: 10px; border-bottom: 1px solid #1a3a1a;">
        <div style="font-size: 13px; color: #8dc88d; font-weight: bold; margin-bottom: 3px;">renderer-raw-svg — Self-Test</div>
